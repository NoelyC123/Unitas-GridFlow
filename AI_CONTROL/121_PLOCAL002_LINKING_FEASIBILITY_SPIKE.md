...
Important note: the final audit confirms the folder-level evidence set is complete. Detailed linking analysis is complete for the strongest evidence cases and partial/conservative for thinner note records. The `pole_notes.md` files for poles 02, 07, and 10 still contain draft placeholder text showing `Support UNKNOWN` and old missing-evidence bullets. This spike treats those notes as incomplete and does not infer pole FID, SPN, equipment, or conductor links from the folder name alone.
...
Poles 02, 07, and 10 have known support numbers at the folder/audit level, but their `pole_notes.md` files are still draft placeholders. For each of these poles: evidence folder is complete per audit, but `pole_notes.md` requires QA enrichment before this pole is used for per-pole linking decisions. Stage 6A should not treat those notes as confirmed pole identity evidence until they are reconciled with the final audit and ENWL screenshots.
